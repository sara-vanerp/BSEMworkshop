...
## Step 2: Change the priors (1)
```{r priors1, cache = TRUE, results = 'hide', message = FALSE, warning = FALSE}
mod1 <- '
  # measurement model
    ind60 =~ x1 + x2 + x3
    dem60 =~ y1 + y2 + y3 + y4
...
```{r priors2, cache = TRUE, results = 'hide', message = FALSE, warning = FALSE}
mcmc_areas(na.omit(samples1),
           pars = pars[29])
...
```{r}
plot(fit, pars = c(9:11), plot.type = "areas")
```

## Step 5: Check the results (2)
```{r}
plot(fit, pars = c(1:8), plot.type = "intervals")
```
